...
Hashed string | A string which is hashed using the algorithm specified in the [project settings](/refguide/project-settings/#hash-algorithm). This can be used to store password hashes, for example, so that the original password is not recorded in the database.  | String |
