...
This document describes imported web services. If you're looking for specific information on the consumed web services screen, you can check the [Consumed web service](/refguide/consumed-web-service/) documentation.
...
To expose functionality in a Mendix server (thus enabling other systems to make use of certain functions) a Microflow can easily be published as a web service. See [Published Web Services](/refguide/published-web-services/).
...
See [XML Schemas](/refguide/xml-schemas/) for information on importing XSDs into your application.
See  [Import Mappings](/refguide/import-mappings/) for info on mapping XML documents to domain entities and [Export Mappings](/refguide/export-mappings/) for info on exporting domain entities as XML.
...
To actually use these imported web services in a microflow, see [Call Web Service Action](/refguide/call-web-service-action/).

## Proxies

If you are behind a firewall, you might need to use a proxy to call a webservice. Specific information on how to configure the JVM to use a proxy can be found [here](/refguide/using-a-proxy-to-call-a-webservice/)
