...
Member defines for which attribute or association the message will be shown. In the case of a [reference selector](/refguide/reference-selector/) or [reference set selector](/refguide/reference-set-selector/) you should select the association that is edited with the selector.
...
For each parameter in the template you define an expression of which the value will be inserted at the position of the parameter. Parameters need to be entered using [expressions](/refguide/expressions/) resulting in a string.
