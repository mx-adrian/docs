...
* **Workflow** – A  that should be executed.
...
**Show Task Page** opens an overview page set for the  in properties. An element with this on-click action should be placed in a data container connected to the **System.WorkflowUserTask** entity. 
...
* **Task** – The  that should be marked as completed.

* **Outcome** – Lists the outcomes of the selected  and follows the selected outcome. If the user task has only one outcome, the **Default** is set as an outcome and the property cannot be edited. 
...
Behavior of the **Delete Object** action depends on a data container it is placed in: a  or a . 
