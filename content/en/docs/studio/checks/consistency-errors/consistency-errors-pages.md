...
If you do not configure a data source for a  properly, you will get a consistency error. For example, you selected a list widget as the data source, but you have not selected the specific list you would like the data view to listen to.
...
| CE0529     | The selected {Name of the page} expects an object of type {type of object} and cannot be used as a home page. Change the page or use a microflow to provide the page with an object. | You have set a page that expects an object to be passed to it (for example, a page with a data view) as the home page. But by default the home page has no object that is passed to it, because it is the starting point for your user. For a more detailed example, see the  section in *Navigation Consistency Errors*. | Set a different  page as the home page. Alternatively, you can use a microflow that will open the home page and pass a specific object to it. For more information, see the  section in *Navigation Consistency Errors*. |
...
The most common errors for  (such as, a text box, a drop-down, a check box, etcetera) , their causes, and ways to fix them are described in the table below. 
...
The most common consistency errors for  can occur when you place an image widget on a page, but do not select an actual image for it. 

 need to be placed inside a data container (a data view or a list view) and an entity should be selected for them. 
...
You can specify an **On Click Action** for different widgets, for example, for buttons or images. For more details about on click actions, see .
