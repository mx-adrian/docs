...
This document describes workflow properties. For details on what workflows are for and what kind of elements you can use there, see .
...
This entity should be of a workflow entity type. For more information, see the  section in the *Domain Model*.
...
**Allowed roles** defines the  that can trigger the workflow.

![Permissions Section](/attachments/studio/workflows/workflow-properties/permissions.jpg)

{{% alert type="info" %}}
This section is only displayed when the security is on. For more information, see .
{{% /alert %}}
...
| Expression | You can set a due date for the workflow writing an expression. For example, to set a due date to tomorrow, you can use `addDays(. |

### 2.5 Admin Page Section {#admin-page}

**Override Admin Page** is an optional page used for displaying all running workflows to the workflow admin. This overrides a generic page used to display any running workflow in the app, for example, when you have a **Show Workflow Page** set as an  or as a  and would like to override the page selected for this event/activity. 
